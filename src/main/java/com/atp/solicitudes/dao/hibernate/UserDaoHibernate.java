...
public class UserDaoHibernate extends HibernateDaoTemplate implements UserDao
{
	...
	public DaoResult<User> query(DaoQuery query, DaoOrder order)
	{
		...
		String[] eqs = {"id", "status", "profile","empresa","reportAccessLevel","pantalla"};
		registerEqPaths(eqs, query, criteria);

		System.out.println("-----consulta---------" );
		String[] likes = {"username", "email","validity_date","userN4_id_vali"};
	...
	public List<String> getColumnPropertyValues_(String v,String id_N4_user)throws Exception
	{
		System.out.println("-----id----------   " + id_N4_user);
		Query query = getSession().createQuery("select model.nivel from UserTransportistas as model where model.transportista_id = "+ id_N4_user +" and model.nivel ="+ "4" +"");
		@SuppressWarnings("unchecked")
