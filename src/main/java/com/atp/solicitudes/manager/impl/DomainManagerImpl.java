...
public class DomainManagerImpl extends AppContextProvider implements DomainManager, ActivityLogEntryRegister
{
	...
	@Resource(name="BloqueosCitasDao")
	private BloqueosCitasDao BloqueosCitasDao;
	...
	@Resource(name="solicitudContenedorDao")
	private SolicitudContenedorDao solicitudContenedorDao;
	
	@Resource(name="userTransportistasDao")
	private UserTransportistasDao userTransportistasDao;
	...
	public List<String> getUsersColumnPropertyValues_(String id,String id_N4_user) throws Exception
	{
		return userDao.getColumnPropertyValues_(id,id_N4_user);
	}
	...
	public DaoQuery getQueryWithUserFilter(User user) throws Exception
	{
		...
		{
			return new DaoQuery();//DaoQuery.withFilter("user", user);
		}
	}
	...
	public Boolean shouldNewPasswordBeDefinedFor1(User user) throws Exception
	{
		...
		int totalDays = DateUtils.getDaysBetweenDates(user.getLastPasswordChange(), new Date());
		 ...
		 System.out.println("-doma1-" + getNumberOfDaysPasswordExpire().toString());
		 System.out.println("-doma-" + String.valueOf(totalDays));
	...
	public Boolean shouldNewPasswordBeDefinedFor(User user) throws Exception
	{
        ...
        String fecha_vi =user.getvalidity_date() ;
        
         DateFormat formatoFecha = new SimpleDateFormat("yyyy-MM-dd");
         String fecha_sys =formatoFecha.format(fechaActual);
        
        Boolean v = true;
        
		// if no password already set, define one
        System.out.println("-doma-" +  user.validity_date());
        System.out.println("-doma-" +  formatoFecha.format(fechaActual));
        Calendar fecha = new GregorianCalendar();
       
        int año = fecha.get(Calendar.YEAR);
       
        int dia = fecha.get(Calendar.DAY_OF_MONTH);
        String[] FechaArray = fecha_vi.split("-");
        String[] FechaArray2 = fecha_sys.split("-");
        int mes = Integer.parseInt( FechaArray2[1]);
        int anio= Integer.parseInt( FechaArray[0]);
        int meis= Integer.parseInt( FechaArray[1]);
        int diia= Integer.parseInt( FechaArray[2]);
        
        System.out.println("año" + anio + "--" + año );
        System.out.println("mes" + meis + "--" + mes );
        System.out.println("dia" + diia + "--" + dia );
        
		if ( user.getvalidity_date() == fecha_sys)
		{
			v=false;
			System.out.println("-false-" );
		}
		if (user.getvalidity_date() =="null")
		{
			v=true;
			System.out.println("-true-" );
		}
		if(anio <= año && meis<=mes && diia <=dia  )
		{
			v=false;
			System.out.println("-false-" );
		}
...
//				throw new DomainModelException("User_with_email_already_exists");
	...
	//bloqueos
	public List<Bloqueos> queryBloqueos() throws Exception
	{
		return BloqueosCitasDao.query();
	}
	...
	public DaoResult<Solicitud> getSolicitudesByAdmin(DaoQuery query, DaoOrder order, List<String> UserN4) throws Exception{
		return solicitudDao.queryN4(query, order,UserN4);
	}
...
//			throw new DomainModelException("Solicitud_status_revokes_edition");
	...
	public List<String> getSolicitudByN4(String userN4, String idUser) throws Exception{
		return solicitudDao.getSolicitudByN4(userN4,idUser);
	}	
	
	public List<String> getSolicitudByN4(String idUser) throws Exception{
		return solicitudDao.getSolicitudByN4(idUser);
	}
	
	//prueba
	public List<String> getSolicitudByN4_(String User) throws Exception{
		return solicitudDao.getSolicitudBy_(User);
	}
	
	public List<String> prueba(String User) throws Exception{
		return solicitudDao.getSolicitudBy_(User);
	}
	public List<String>  getSolicitudAppointmentWithcontenedor(String user, String contenedor) throws Exception
	{
		return solicitudDao.getWithcontenedor(user, contenedor);
	}
	
	public List<String>  getSolicitudAppointmentWithcontenedorAA(String user, String contenedor) throws Exception
	{
		return solicitudDao.getWithcontenedorAA(user, contenedor);
	}
	
	public List<String>  getSolicitudAppointmentWithcontenedorcon(String user, String contenedor) throws Exception
	{
		return solicitudDao.getWithcontenedorcon(user, contenedor);
	}
	
	public List<String>  getSolicitudAppointmentWithcontenedortra(String user, String contenedor) throws Exception
	{
		return solicitudDao.getWithcontenedortran(user, contenedor);
	}
	public List<String>  getdocumentos(String bls, String impe,String li,String fe,String u) throws Exception
	{
		return solicitudDao.getdocumentos(bls, impe,li,fe,u);
	}
	public List<String>  getdocumentostodos(String bls, String impe,String li,String fe,String u) throws Exception
	{
		return solicitudDao.getdocumentostodos(bls, impe,li,fe,u);
	}
	public List<String>  getdocumentoscon(String bls, String impe,String li,String fe,String u) throws Exception
	{
		return solicitudDao.getdocumentoscon(bls, impe,li,fe,u);
	}
	public List<String>  getdocumentostra(String bls, String impe,String li,String fe,String u) throws Exception
	{
		return solicitudDao.getdocumentostra(bls, impe,li,fe,u);
	}
	public List<String>  getpedimentos(String bls) throws Exception
	{
		return solicitudDao.getpedimentos(bls);
	}
	...
	public SolicitudAppointment getSolicitudAppointmentWithAppNbr(String app_nbr, List<String> listid) throws Exception
	{
		return getSolicitudAppointmentDao().getWithAppNbr(app_nbr, listid);
	}
	...
	public void saveSolicitudAppointment(SolicitudAppointment solicitudAppointment) throws Exception
	{
		...
		publishEvent(new ActivityEvent(
				ActivityLogEntry.createForActionAndAppender(ActivityLogEntryConstants.SOLICITUD_APPOINTMENT_CREATED, solicitudAppointment)));
	}
	...
	public List<SolicitudAppointment> getSolicitudAppointmentFromSolicitudByTransporter(Solicitud solicitud,Integer idTransporter) throws Exception
	{
		DaoQuery query = DaoQuery.withFilter("solicitud", solicitud);
		DaoOrder order = DaoOrder.withSort("dateUpdated", DaoOrder.DESC);

		DaoResult<SolicitudAppointment> result = getSolicitudAppointmentDao().query(query, order,idTransporter);
	...
	public List<String> getSolicitudesByUserN4(Integer userN4) throws Exception{
		return solicitudAppointmentDao.getSolicitudesByUserN4(userN4);
	}
	...
	public void saveSolicitudComentarioAndNotify(SolicitudComentario obj) throws Exception
	{
		...
		Integer agenteaduanal =solicitud.getAgenciaAduanalId();
		OperationValidationStateEnum statc=solicitud.getStatus();
		String a=statc.getName();
		...
		AgenciaAduanal agente=managerN4.getAgenciaAduanalWithId(agenteaduanal);
        Empresa empresa;
		empresa=commentUser.getEmpresa();
		 System.out.print("pruebitas "+statc + "----"+a+"tu "+solicitud.getStatus());
		...
		mailParams.put("subject", getMailNotificationSubjectHeader() +" Folio:"+solicitud.getFolio()+" Usuario:"+ commentUser.getUsername());
		...
		modelParams.put("agenteaduanal", agente.getNombre());
        modelParams.put("status",solicitud.getStatus());
	...
	public List<String> getDocumentoSolicitudbyId1(Integer id)  throws Exception
	{
		return documentoSolicitudDao.getDocumentsWithId(id);
	}
	...
	// SolicitudContenedor
		...
		public SolicitudContenedor getSolicitudContenedorWithId(Integer id) throws Exception
		{
			return solicitudContenedorDao.getWithId(id);
		}

		@Transactional(readOnly = false, rollbackForClassName = { "Exception" })
		public void saveSolicitudContenedor(SolicitudContenedor obj) throws Exception
		{
			solicitudContenedorDao.save(obj);
		}
		
		public DaoResult<SolicitudContenedor> getSolicitudContenedoresForSolicitud(Solicitud solicitud) throws Exception
		{
			DaoQuery query = DaoQuery.withFilter("solicitud", solicitud);
			DaoOrder order = DaoOrder.withSort("fecha", DaoOrder.ASC);
			
			return solicitudContenedorDao.query(query, order);
		}
		
		@Transactional(rollbackForClassName = { "Exception" })
		public void deleteSolicitudContenedor(SolicitudContenedor solicitudContenedor) throws Exception
		{
			solicitudContenedorDao.delete(solicitudContenedor);
		}
		
		@Transactional(rollbackForClassName = { "Exception" })
		public void updateSolicitudContenedor(SolicitudContenedor solicitudContenedor) throws Exception
		{
			solicitudContenedorDao.update(solicitudContenedor);
		}
	
		// UserTransportistas

		public UserTransportistas getUserTransportistasWithId(Integer id) throws Exception
		{
			return userTransportistasDao.getWithId(id);
		}

		@Transactional(readOnly = false, rollbackForClassName = { "Exception" })
		public void saveUserTransportistas(UserTransportistas obj) throws Exception
		{
			userTransportistasDao.save(obj);
		}
		
		public DaoResult<UserTransportistas> getUserTransportistasForUser(User solicitud) throws Exception
		{
			DaoQuery query = DaoQuery.withFilter("solicitud", solicitud);
			DaoOrder order = DaoOrder.withSort("fecha", DaoOrder.ASC);
			
			return userTransportistasDao.query(query, order);
		}
		
		@Transactional(rollbackForClassName = { "Exception" })
		public void deleteUserTransportistas(UserTransportistas userTransportistas) throws Exception
		{
			userTransportistasDao.delete(userTransportistas);
		}
		
		@Transactional(rollbackForClassName = { "Exception" })
		public void updateUserTransportistas(UserTransportistas userTransportistas) throws Exception
		{
			userTransportistasDao.update(userTransportistas);
		}
		
		public List<UserTransportistas> getUserTransportistasbyUserN4(Integer UserN4) throws Exception
		{
			return userTransportistasDao.getUserTransportistasbyUserN4(UserN4);
		}
		
		public UserTransportistas getUserTransportistasWithIdN4Transport(Integer id,Integer userN4,Integer idTrnas) throws Exception
		{
			return userTransportistasDao.getUserTransportistasWithIdN4Transport(id,userN4,idTrnas);
		}
		
		
		public List<UserTransportistas>getnombresByAdmin(List<SimpleEntry> listObj) throws Exception
		{
			return userTransportistasDao.getnombresByAdmin(listObj);
		}
		
		public List<String>getContenedorStatus(String id) throws Exception
		{
			return userTransportistasDao.getContenedorStatus(id);
		}
		//bloqueos
		
		public ArrayList<Bloqueos> getBloqueosStatus(String strtipo) throws Exception
		{
			return BloqueosCitasDao.getBloqueosStatus(strtipo);
		}
		public ArrayList<Bloqueos> getBloqueosStatushabilitar(String strtipo,String st,String s) throws Exception
		{
			System.out.print("prueba"+s);
			return BloqueosCitasDao.getBloqueosStatushabilitar(strtipo,st,s);
		}
		@Transactional(readOnly = false, rollbackForClassName = { "Exception" })
		public Bloqueos processSaveUpdateBloqueos(JsonNode node) throws DomainModelException, Exception
		{
			...
			{
				// retrieve holiday
				Bloqueos = getBloqueosWithId(node.get("id").asInt());
			}
			SimpleDateFormat simpleDateFormat = (SimpleDateFormat) getBean("dateFormatter_yyyy-MM-dd");
			...
			//Date fec = new Date();
			//String fechaEnviar = formatoDelTexto.format(fec);
		 	//Bloqueos.setFecha_creacion(fechaEnviar);
			//Bloqueos.setFecha_actualizacion(fechaEnviar);
			
			Bloqueos.fillFrom(node, context);
		...
		@Transactional(rollbackForClassName = { "Exception" })
		public void deleteBloqueos(Bloqueos userBloqueos) throws Exception
		{
			BloqueosCitasDao.delete(userBloqueos);
		}
		
		@Transactional(rollbackForClassName = { "Exception" })
		public void updateBloqueos(Bloqueos userBloqueos) throws Exception
		{
			BloqueosCitasDao.update(userBloqueos);
		}
		
		
		public Bloqueos getBloqueosWithId(Integer id) throws Exception
		{
			return BloqueosCitasDao.getWithId(id);
		}
	
		@Transactional(readOnly = false, rollbackForClassName = { "Exception" })
		public void saveBloqueos(Bloqueos obj) throws Exception
		{
			BloqueosCitasDao.save(obj);
		}
	...
	public DaoResult<ActivityLogEntry> queryActivityLogEntryByUser(DaoQuery query, DaoOrder order,String user) throws Exception
	{
		return activityLogEntryDao.query(query, order,user);
	}
	
	public DaoResult<ActivityLogEntry> queryActivityLogEntryByUsers(DaoQuery query, DaoOrder order,List<String> users) throws Exception
	{
		return activityLogEntryDao.query(query, order,users);
	}
	...
	public SolicitudContenedorDao getSolicitudContenedorDao() {
		return solicitudContenedorDao;
	}

	public void setSolicitudContenedorDao(
			SolicitudContenedorDao solicitudContenedorDao) {
		this.solicitudContenedorDao = solicitudContenedorDao;
	}
