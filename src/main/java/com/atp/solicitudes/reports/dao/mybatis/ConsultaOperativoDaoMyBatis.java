...
public class ConsultaOperativoDaoMyBatis extends MyBatisDaoTemplate implements ConsultaOperativoDao
{
	...
	public DaoResult<ConsultaOperativo> getConsultaExportacion(String buqueViaje) throws Exception
	{
		DaoResult<ConsultaOperativo> result = new DaoResult<ConsultaOperativo>();
		Map<String, Object> map = new HashMap<String, Object>();
		String[] variable = buqueViaje.split(" ");
		String buque = variable[2];
		map.put("buque_viaje","%"+buque);
		List<ConsultaOperativo> col = this.<ConsultaOperativo> selectList("mybatis.reports.ConsultaOperativoMapper.selectConsultaOperativoExportacion", map, new RowBounds());
		result.setCollection(col);
